...
const readRow = function (parser, tablemap, bitmap, zongji) {
  if (!tablemap || !tablemap.columns) {
    console.warn('Skipping row for missing tableMap or columns');
    // Avanzamos el parser al final del paquete
    parser._offset = parser._packetEnd;
    return {};
  }
  ...
  for (let i = 0, nullBitIndex = 0; i < tablemap.columns.length; i++) {
    ...
    if (getBit(bitmap, i) == 0) {
      row[column.name || `column_${i}`] = null;
      continue;
    }
    ...
    if (getBit(nullBitmap, nullBitIndex) != 0) {
      ...
      row[column.name || `column_${i}`] = Common.readMysqlValue(parser, column, columnSchema, tablemap, zongji);
    }
